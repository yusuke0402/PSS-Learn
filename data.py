...
class DataSets:
    ...
    target_split = int(
        config["hyperparam"]["split_ratio"] * config["datasettings"]["target_number"]
    )
    ...
    def __init__(self):
        ...
        self.__current_number = DataSets.config["datasettings"][
            "target_number"
        ]  # 現在試験の被験者数
        self.__current_x, self.__current_x_mean = generate_multivariate_normal(
            "A", size=self.__current_number
        )  # 現在試験の共変量　行が被験者、列が共変量
        self.__epsilon = np.random.normal(
            loc=0, scale=1, size=self.__current_number
        )  # 測定誤差
        self.__current_y = truefunction(
            x=self.__current_x, t=np.ones(self.__current_number), epsilon=self.__epsilon
        ).reshape(-1, 1)  # 現在試験のアウトカム　列ベクトル

        self.__historical_1_number = DataSets.config["datasettings"]["source1_number"]
        self.__historical_1_x, self.__historical_1_x_mean = (
            generate_multivariate_normal("B", size=self.__historical_1_number)
        )
        self.__epsilon = np.random.normal(
            loc=0, scale=1, size=self.__historical_1_number
        )
        self.__historical_1_y = truefunction(
            x=self.__historical_1_x,
            t=np.zeros(self.__historical_1_number),
            epsilon=self.__epsilon,
        ).reshape(-1, 1)

        self.__historical_2_number = DataSets.config["datasettings"]["source2_number"]
        self.__historical_2_x, self.__historical_2_x_mean = (
            generate_multivariate_normal("B", size=self.__historical_2_number)
        )
        self.__epsilon = np.random.normal(
            loc=0, scale=1, size=self.__historical_2_number
        )
        self.__historical_2_y = truefunction(
            x=self.__historical_2_x,
            t=np.zeros(self.__historical_2_number),
            epsilon=self.__epsilon,
        ).reshape(-1, 1)

        self.__historical_x = np.concatenate(
            (self.__historical_1_x, self.__historical_2_x), axis=0
        )  # 履歴試験の共変量
        self.__historical_y = np.concatenate(
            (self.__historical_1_y, self.__historical_2_y), axis=0
        )  # 履歴試験のアウトカム
